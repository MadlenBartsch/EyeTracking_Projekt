...
def generate_all_trials(image_combos):
    ...
    for combo in image_combos:
        ...
        trials.append({
            "image1": attr_img,
            "image2": comparison_img,
            "attribute": attr,
            "value": val,
            "target_image": attr_img,
            "cb_index": f"{attr}_{val}_L"
        })

        trials.append({
            "image1": comparison_img,
            "image2": attr_img,
            "attribute": attr,
            "value": val,
            "target_image": attr_img,
    ...
    for cat1, cat2 in cat_pairs:
        ...
        for img1, img2 in img_pairs:
            trials.append({
                ...
                "attribute": "none",
                "value": "none",
                "target_image": random.choice([img1, img2]),
...
def generate_subject_lists_balanced():
    ...
    for i, subject_trials in enumerate(balanced_lists):
        ...
        with open(filename, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=["image1", "image2", "attribute", "value", "target_image", "cb_index"])
            writer.writeheader()
...
def log_image_distribution(subject_lists):
    usage_counts = defaultdict(lambda: {"as_image1": 0, "as_image2": 0, "as_target": 0, "as_comparison": 0})
    ...
    for trials in subject_lists:
        for trial in trials:
            ...
            usage_counts[img1]["as_image1"] += 1
            usage_counts[img2]["as_image2"] += 1
